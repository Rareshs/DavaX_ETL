...
INPUT_FILE = os.path.join(INPUT_DIR, "Cluj.csv")
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "Cluj_abs.csv")
...
df[['SDATE', 'SHOUR']] = df['SDATE'].astype(str).str.extract(r'^\s*([0-9/]+)\s+(.+)$')
df[['EDATE', 'EHOUR']] = df['EDTATE'].astype(str).str.extract(r'^\s*([0-9/]+)\s+(.+)$')
